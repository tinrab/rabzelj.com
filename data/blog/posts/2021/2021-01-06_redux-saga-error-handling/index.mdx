...
Let `Transaction` class hold the state of a transaction, which can be _in progress_, end in _failure_, or end _successfully_.
...
export const userSelectors = {
  currentUser: (state: AppState): UserData | undefined => state.user.currentUser,
  signInTransaction: (state: AppState): Transaction => state.user.signInTransaction,
};
```
