...
export const signInAction = createApiAction(signInFormSchema, async (data): Promise<void> => {
...
export function createApiAction<TSchema extends z.ZodTypeAny, TResult = void>(
  ...
  return async (data: unknown): Promise<ActionResponse<TResult>> => {
    const parsedData = schema.safeParse(data instanceof FormData ? Object.fromEntries(data) : data);
    if (parsedData.error) {
      return {
...
export async function callApiAction<TSchema extends z.ZodTypeAny, TResult = void>(
