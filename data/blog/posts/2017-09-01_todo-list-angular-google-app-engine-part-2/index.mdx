...
Navigate to [http://localhost:4200](http://localhost:4200) with your browser. Also keep the App Engine server running.
...
```typescript {highlight:3,6,13,16}
...
```typescript {highlight:1,13}
import { TodoService } from "./todo.service";
@NgModule({
  declarations: [AppComponent, SignInComponent, ListComponent],
  imports: [BrowserModule, HttpClientModule, AppRoutingModule],
...
```typescript {highlight:1,5-6,13,15,19,23-26}
...
```html
<div>
  <span *ngIf="!editMode; else titleInput" (click)="onTitleClick()">{{item.title}}</span>
...
```typescript {highlight:4-5,16-19,21,23-35}
...
```typescript {highlight:3-4,7,10-12}
export class TodoComponent {
  // ...
  @Output()
...
<button md-raised-button class="sign-in-button" color="primary" (click)="signIn()">Sign in</button>
```
...
```html
<md-list-item>
  <span md-line *ngIf="!editMode; else titleInput" (click)="onTitleClick()">{{item.title}}</span>
  <ng-template #titleInput>
